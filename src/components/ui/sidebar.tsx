...
>(
  (
    ...
    if (!isMounted) {
      // Render a static version on the server to prevent hydration mismatch
      return (
        <SidebarContext.Provider value={{...contextValue, open: defaultOpen, state: defaultOpen ? 'expanded' : 'collapsed'}}>
      ...
      >
        ...
        >
          <div
            ...
            className="flex h-full w-full flex-col bg-card rounded-lg"
          >
            {children}
          </div>
        </div>
